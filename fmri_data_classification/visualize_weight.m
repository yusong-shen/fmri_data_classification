...
imagesc(W1_norm2);
colorbar
fig_name = sprintf('saves/w1_%ditr.fig',i);
savefig(fig_name);
...
table_name = sprintf('saves/W1_norm_sorted_%ditr.mat',i);
save(table_name, 'W1_sorteds');
% output the matrix to excel xls file
xls_name = sprintf('W1_norm_sorted_%ditr',i);
xlswrite(xls_name, W1_sorteds);
...
imagesc(W2_norm2);
colorbar
fig_name = sprintf('saves/w2_%ditr.fig',i);
savefig(fig_name);
...
% bar(W1_norm3);
% [val, ind] = max(W1_norm3);
table_name = sprintf('saves/W2_norm_sorted_%ditr.mat',i);
save(table_name, 'W2_sorteds');
% output the matrix to excel xls file
xls_name = sprintf('W2_norm_sorted_%ditr',i);
xlswrite(xls_name, W2_sorteds);
